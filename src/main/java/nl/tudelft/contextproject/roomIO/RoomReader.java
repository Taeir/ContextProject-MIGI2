...
public final class RoomReader {
	...
	public static File getMapFile(String path) throws FileNotFoundException {
		File folder = new File(RoomReader.class.getResource(path).toURI());
		File[] files = folder.listFiles();
		if (files == null) throw new FileNotFoundException(path + " is not a folder.");
		for (int i = 0; i < files.length; i++) {
			if (files[i].getName().endsWith(".crf")) return files[i];
		}
		throw new FileNotFoundException("Could not find a '.crf' file in " + folder.getAbsolutePath() + ".");
	}
}
